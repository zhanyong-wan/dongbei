...
dongbei 语言是基于 Python 二次开发的。只要能跑 Python 的旮旯儿都能跑。像 Mac OS 啦、Windows 啦、Linux 啦，等等等等，都成！
...
用 utf-8 编码保存。要是编辑器因为编码错误埋汰你，把文件内容改成
...
学习一门语言，先得了解它的**词法**（怎么从一串串的字符组成词），然后是**语法**（怎么把词组成句子）和**语义**（这些句子都啥意思啊？）。所以，咱们先讲讲 dongbei 语言词汇的构成。
...
一行代码当中，如果在字符串常量*外面*出现 **#** 字符，所以从 # 开始的字符都会被当成注释被忽略掉。比如
