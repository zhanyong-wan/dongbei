...
桃园社 装 老刘 跟 老关 跟 老张 抱团。  # (老刘, 老关, 老张)
嘀咕：桃园社 有几个坑。  # 3
```
...
“滚犊子吧”有它的局限性，那就是滚完之后套路就整不下去了。
有滴时候捏，俺们在一个套路里头滚完还想再滚，连滚带爬那种。
这又该咋整捏？

这个就涉及到一种全新的出行方式了：“出溜”。
出溜跟滚犊子的区别，在于出溜完了吧还可以在套路里接着出溜。
一直到套路弄完了，或者是使套路的银半途而废为止。
...
所谓平方数，就是从一一得一，二二得四，三三得九开始，一直向下数不停。
就好了。
写成 dongbei 语言，揍是这样的：
...
每次一出溜，就会从套路里蹦出来（数学老师 乘 数学老师）的当前值。
这个值捏，一开始是 1，然后是4，9，16......

接下来是提问时间：如何利用这个平方数的套路打印出100以内的平方数捏？
这个难不倒小明：
